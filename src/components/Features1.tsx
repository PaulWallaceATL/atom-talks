      ...
      <div className="container position-relative" style={{ zIndex: 1 }}>
        <FadeInSection>
          <div className="text-center mb-18">
            <h1 className={`mb-0 text-${dark ? 'white' : 'dark'}`}>
              Why Choose Our <span className={`text-${dark ? 'primary-dark' : 'primary'}`}>AI Agent Platform</span>
            </h1>
          </div>
        </FadeInSection>

        <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-6 g-xl-10">
          {features.map((feature, index) => (
            <div key={index} className="col">
              <FadeInSection delay={index * 0.1}>
                <CardSpotlight className="h-100">
                  <div className="d-flex flex-column gap-6 p-4">
                <div className={`icon w-14 h-14 flex-shrink-0 d-flex align-center justify-center rounded-3 p-2 border bg-gradient-3 text-${dark ? 'primary-dark' : 'primary'} border-white border-opacity-10`}>
                  {feature.icon}
                </div>
                    <div className="content">
                      <h4 className={`mb-4 text-${dark ? 'white' : 'dark'}`}>{feature.title}</h4>
          ...
          <div className="col-12">
            <FadeInSection delay={0.6}>
              <CardSpotlight>
                <div className="d-flex flex-column flex-lg-row gap-6 align-items-center justify-content-center text-center text-lg-start p-6 p-lg-8 bg-dark-blue-3 rounded-4">
              <div className={`icon w-14 h-14 flex-shrink-0 d-flex align-center justify-center rounded-3 p-2 border bg-gradient-3 text-${dark ? 'primary-dark' : 'primary'} border-white border-opacity-10`}>
                <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 40 40">
                  <g stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2">
                    <path d="M20 35c8.284 0 15-6.716 15-15 0-8.284-6.716-15-15-15-8.284 0-15 6.716-15 15 0 8.284 6.716 15 15 15Z" />
                  ...
                  <div className="content">
                    <h4 className={`mb-4 text-${dark ? 'white' : 'dark'}`}>Drop-In Widget. Enterprise Ready.</h4>
                    <p className="mb-0">Easy install. Secure, customizable, and integrates with your GTM stack.</p>
                  </div>
                </div>
              </CardSpotlight>
            </FadeInSection>
